...
export async function extractPDFWithAzure(file: File, options?: AzureOptions): Promise<AzureExtractionResult> {
    ...
    const client = DocumentIntelligence(AZURE_EXTRACT_INFO.endpoint, new AzureKeyCredential(AZURE_EXTRACT_INFO.apiKey));
...
async function postAnalyzeRequest(
    ...
    while (postAttempts < AZURE_EXTRACT_INFO.MAX_POST_ATTEMPTS) {
        const response = (await client.path('/documentModels/{modelId}:analyze', 'prebuilt-layout').post({
        ...
        if (is429Status || is429Body) {
            const waitTime = calculateRetryWait(response.headers['retry-after'], response.body?.error?.message);
 ...
 */
function parseTableRegions(analyzeResult: AzureAnalyzeResult, pageHeightMap: Map<number, number>): TableRegion[] {
    return (
        analyzeResult.tables?.map((table) => {
